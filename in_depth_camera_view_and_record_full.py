...
#
#
# # Function to determine squat state
# def leg_state(angle):
#     if angle < 0:
...
#
#
# # Initialize single video and JSON output for all squats
# def initialize_output(output_dir):
#     video_output_path = os.path.join(output_dir, "all_squats_video.mp4")
#     json_output_path = os.path.join(output_dir, "all_squats_landmarks.json")
...
#         return None, None
#
#     return out, json_output_path
#
#
# # Record landmarks data for each frame
# def record_landmarks(frame_count, results, landmark_data_json, squat_count):
...
#             })
#     landmark_data_json.append({
#         "frame": frame_count,
#         "squat_count": squat_count,
#         "landmarks": landmarks_frame_data
#     })
#
#
# # Process each frame and return depth and color images along with pose results
...
#
#
# # Display tracking status on the image
...
#
#
# # Draw pose landmark connections on the image
...
#
#
# # Save video frame and display frame number
# def save_output(video_writer, annotated_image, frame_count):
#     # Draw frame number on the bottom-right corner of the screen
#     cv2.putText(annotated_image, f"Frame: {frame_count}",
#                 (annotated_image.shape[1] - 200, annotated_image.shape[0] - 20),
...
#
#
# # Check body visibility based on landmarks
...
#         return len(visible_landmarks) >= min_visible_landmarks
#     return None
#
#
# # Save single JSON file for all squats
# def save_all_squat_output(landmark_data_json, json_output_path):
#     with open(json_output_path, 'w') as json_file:
#         json.dump(landmark_data_json, json_file, indent=4)
#
#
# # Main function to run squat counting and recording
# def main():
...
#
#     # Initialize one video file and one JSON file for all squats
#     video_writer, json_output_path = initialize_output(output_dir)
#     if video_writer is None:
#         print("Video writer initialization failed. Exiting.")
#         return
#
#     landmark_data_json = []
#
...
#
#             if results.pose_landmarks:
#                 # Record landmarks for each frame
#                 record_landmarks(frame_count, results, landmark_data_json, squat_count)
#
#                 r_knee_angle = find_angle(results.pose_landmarks.landmark[24], results.pose_landmarks.landmark[26],
#                                           results.pose_landmarks.landmark[28])
#                 l_knee_angle = find_angle(results.pose_landmarks.landmark[23], results.pose_landmarks.landmark[25],
...
#                     last_state = 3
#
#             # Draw the squat count and frame number on the screen
#             draw_squat_count(annotated_image, squat_count)
#
#             # Save video frame for all squats with frame count displayed
#             save_output(video_writer, annotated_image, frame_count)
#
...
#             frame_count += 1
#     finally:
#         # Save the entire JSON data at the end of the session
#         save_all_squat_output(landmark_data_json, json_output_path)
